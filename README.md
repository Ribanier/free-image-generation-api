...
### 3. JavaScript / Browser Client (`fetch`)
You can directly call the API in any frontend web application or browser console:

```javascript
// ⚙️ Configuration Variables
const API_URL = "https://<your-worker-url>";
const API_KEY = "your-secret-api-key"; // Enter your key directly (Bearer prefix is added automatically)
const PROMPT  = "A futuristic city in the clouds";

// 🛡️ Automatically formats the 'Bearer ' token safely
const authHeader = API_KEY.trim().startsWith("Bearer ") 
  ? API_KEY.trim() 
  : `Bearer ${API_KEY.trim()}`;
...
try {
  ...
  console.log("✅ Image generated successfully and added to the page!");
} catch (err) {
